...
def handle_transcription():
    ...
    if file:
        ...
        file_path = os.path.join('/mnt/d/projects/TreeHacks/treehacks2025/src/backend/uploads', filename)
        file.save(file_path)
        ...
        try:
            ...
            summary_from_transcript = summary(transcript)
            ...
            # Get user info from session (for personalized context)
            ...
            return jsonify({
                ...
                "summary": summary_from_transcript
            }), 200
...
def action_items(transcript):
    """
    Generate a list of action items for a doctor's visit transcript.
...
def summary(transcript):
    """
    Generate a summary for a doctor's visit transcript.

    :param transcript: The transcript text of the consultation.
    :return: Summary generated by the chat model.
    """
    prompt = (
        "You are going to get an audio transcript of a doctor's visit for diabetes. "
        "In the audio transcript, there is audio of both the doctor and patient. "
        "You are basically a medically educated scribe assistant for the doctor. "
        "The patient wants a short summary based on the doctor's visits."
    )
    completion = client.chat.completions.create(
        model="gpt-4o",
        messages=[
